...
<section id="app" class="hidden">
  <div class="top">
    ...
    <div class="actions">
      ...
      <button id="venues" class="primary">Verify next available</button>
...
$("#venues").onclick=async()=>{
  const button=$("#venues");
  button.disabled=true;
  $("#status").textContent="Finding the next 12 eligible future fixtures…";
  try{
    const data=await api("/private/api/enrich-venues",{
      method:"POST",
      body:JSON.stringify({window_days:90,limit:12})
    });
    const job=data.job||{};
    activeVenueJobId=data.job_id||job.job_id;
    if(!activeVenueJobId){
